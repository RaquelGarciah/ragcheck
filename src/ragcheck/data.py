...
COLUMNS = ["id", "context", "output", "label", "task_type", "model"]
...
def load_ragtruth(split: str, keep_spans: bool = False) -> pd.DataFrame:
    """Descarga RAGTruth y devuelve columnas [id, context, output, label,
    task_type, model].

    Conserva los nombres originales `context` y `output`, y añade `label` a
    partir de `hallucination_labels`. `split` es "train" o "test". Con
    `keep_spans=True` conserva además la columna cruda `hallucination_labels`
    (la necesita el descriptivo de spans).
    """
